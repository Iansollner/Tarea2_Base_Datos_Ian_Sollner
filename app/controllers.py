...
class TravelController(Controller):
    ...
    @get("/{travel_id:int}/activities", return_dto=ActivityReadFullDTO)
    async def list_travel_activities(self, activity_repo: ActivityRepository, travel_id: int) -> list[Activity]:
        try:
            activities = activity_repo.list(CollectionFilter(field_name="travel_id", values=[travel_id]))
            return activities
        except NotFoundError as e:
            raise NotFoundException(detail=f"Actividades para viaje {travel_id} no encontradas") from e
    ...
    @delete("/{travel_id:int}/users/{user_id:int}",  return_dto = UserReadDTO)
    ...
    async def list_travel_accommodations(self, accommodation_repo: AccommodationRepository, travel_id: int) -> list[Accommodation]:
        try:
            ...
            raise NotFoundException(detail=f"Alojamientos para viaje {travel_id} no encontrados") from e
    ...
    async def list_travel_transports(self, transport_repo: TransportRepository, travel_id: int) -> list[Transport]:
        try:
            transports = transport_repo.list(CollectionFilter(field_name="travel_id", values=[travel_id]))
            return transports
        except NotFoundError as e:
            raise NotFoundException(detail=f"Transportes para viaje {travel_id} no encontrados") from e
    ...
    async def list_travel_expenses(self, expense_repo: ExpenseRepository, travel_id: int) -> list[Expense]:
        try:
            expenses = expense_repo.list(CollectionFilter(field_name="travel_id", values=[travel_id]))
            return expenses
        except NotFoundError as e:
            raise NotFoundException(detail=f"Gastos para viaje {travel_id} no encontrados") from e